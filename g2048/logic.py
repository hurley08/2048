...
def fixed_move(board):
    # Cycles through a predetermined order of move functions until 
...
def random_move(board):
    # Chooses a move function in move_order until either a move is made,
    #the list of moves have been exhausted, or no moves can be made. 
    move_made = False
...
def add_new_tile(board):
    # After a move and existing tiles are merged this will select an available 
    # tile to place a 2 
...
def check_for_loss(board):
    board, moves_left = fixed_move(board)
    winner = check_for_win(board)
    return not moves_left and not winner
