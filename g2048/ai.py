...
def ai_move(board, searches_per_move, search_length):
    """
    ai_move uses a simple simulation approach to explore 
    possible future game states for each possible first move 
    and selects the move that leads to the highest accumulated 
    score after simulations. The final board state after the 
    best move and a boolean indicating the validity of the 
    game are returned.
    """
...
def ai_play(board):
    """
    ai_play uses the ai_move function to make AI-controlled 
    moves, updates the board, and checks for a win at each 
    step. The function continues until the game is no longer 
    valid, printing the board and move number at each step, 
    and finally returning the maximum value on the final board.
    """
    move_number = 0
    valid_game = True
    while valid_game:
...
def ai_plot(move_func):
    ...
    for _ in range(SAMPLE_COUNT):
        ...
        game_is_win = check_for_win(board)
        while not game_is_win and game_is_loss:
